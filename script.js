// ══════════════════════════════════════════════════
//  VinylVibes — script.js (versión localStorage)
//  Sin backend: todo se lee/escribe en localStorage
// ══════════════════════════════════════════════════

// ── DATOS SEMILLA ─────────────────────────────────
const SEED_DISCOS = [
    { discogs_id:'1', titulo:'Abbey Road', artista:'The Beatles', genero:'Rock', precio:29.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/4/42/Beatles_-_Abbey_Road.jpg' },
    { discogs_id:'2', titulo:'Kind of Blue', artista:'Miles Davis', genero:'Jazz', precio:24.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/2/25/Miles_Davis_-_Kind_of_Blue.jpg' },
    { discogs_id:'3', titulo:'Thriller', artista:'Michael Jackson', genero:'Pop', precio:34.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/5/55/Michael_Jackson_-_Thriller.png' },
    { discogs_id:'4', titulo:'Random Access Memories', artista:'Daft Punk', genero:'Electronic', precio:39.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/a/a7/Random_Access_Memories.jpg' },
    { discogs_id:'5', titulo:'To Pimp a Butterfly', artista:'Kendrick Lamar', genero:'Hip Hop', precio:27.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/f/f6/To_Pimp_a_Butterfly.png' },
    { discogs_id:'6', titulo:'Rumours', artista:'Fleetwood Mac', genero:'Rock', precio:26.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/f/f1/Fleetwood_Mac_-_Rumours.png' },
    { discogs_id:'7', titulo:'Blue', artista:'Joni Mitchell', genero:'Folk', precio:23.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/3/3a/Joni_Mitchell_-_Blue.png' },
    { discogs_id:'8', titulo:'Nevermind', artista:'Nirvana', genero:'Rock', precio:28.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/b/b7/NirvanaNevermindalbumcover.jpg' },
    { discogs_id:'9', titulo:'The Miseducation of Lauryn Hill', artista:'Lauryn Hill', genero:'Hip Hop', precio:25.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/a/ac/Lauryn_Hill_-_The_Miseducation_of_Lauryn_Hill.png' },
    { discogs_id:'10', titulo:'Purple Rain', artista:'Prince', genero:'Pop', precio:31.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/9/9e/Princeandtherevolutionpurplerain.jpg' },
    { discogs_id:'11', titulo:'Led Zeppelin IV', artista:'Led Zeppelin', genero:'Rock', precio:27.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/2/26/Led_Zeppelin_-_Led_Zeppelin_IV.jpg' },
    { discogs_id:'12', titulo:'A Love Supreme', artista:'John Coltrane', genero:'Jazz', precio:22.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/5/5c/John_Coltrane_-_A_Love_Supreme.jpg' },
    { discogs_id:'13', titulo:'Blonde on Blonde', artista:'Bob Dylan', genero:'Folk', precio:24.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/a/a2/Bob_Dylan_-_Blonde_on_Blonde.jpg' },
    { discogs_id:'14', titulo:'What\'s Going On', artista:'Marvin Gaye', genero:'Blues', precio:26.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/a/aa/Marvin_Gaye_-_What%27s_Going_On.jpg' },
    { discogs_id:'15', titulo:'Buena Vista Social Club', artista:'Buena Vista Social Club', genero:'Latin', precio:29.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/f/fd/BuenaVistaSocialClubAlbum.jpg' },
    { discogs_id:'16', titulo:'Legend', artista:'Bob Marley', genero:'Reggae', precio:23.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/5/52/Bob_Marley_-_Legend.jpg' },
    { discogs_id:'17', titulo:'The Dark Side of the Moon', artista:'Pink Floyd', genero:'Rock', precio:32.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/3/3b/Dark_Side_of_the_Moon.png' },
    { discogs_id:'18', titulo:'Back in Black', artista:'AC/DC', genero:'Rock', precio:27.99, imagen_url:'https://upload.wikimedia.org/wikipedia/commons/thumb/b/be/Acdc_backinblack.jpg/300px-Acdc_backinblack.jpg' },
    { discogs_id:'19', titulo:'Illmatic', artista:'Nas', genero:'Hip Hop', precio:24.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/5/5b/Illmatic.jpg' },
    { discogs_id:'20', titulo:'Innervisions', artista:'Stevie Wonder', genero:'Blues', precio:25.99, imagen_url:'https://upload.wikimedia.org/wikipedia/en/a/a9/InnervisionsSW.jpg' },
];

const SEED_USUARIOS = [
    { id_usuario:1, nombre:'admin', correo:'admin@vinylvibes.com', password:'admin123', rol:'admin', created_at: new Date('2024-01-01').toISOString() },
    { id_usuario:2, nombre:'demo',  correo:'demo@vinylvibes.com',  password:'demo123',  rol:'demo',  created_at: new Date('2024-01-15').toISOString() },
];

// ── INICIALIZAR DATOS EN localStorage ─────────────
(function seedLocalStorage() {
    if (!localStorage.getItem('vv_discos')) {
        localStorage.setItem('vv_discos', JSON.stringify(SEED_DISCOS));
    }
    if (!localStorage.getItem('vv_usuarios')) {
        localStorage.setItem('vv_usuarios', JSON.stringify(SEED_USUARIOS));
    }
    if (!localStorage.getItem('vv_ventas')) {
        localStorage.setItem('vv_ventas', JSON.stringify([]));
    }
})();

// ── HELPERS DE DATOS ──────────────────────────────
function getDiscos()   { return JSON.parse(localStorage.getItem('vv_discos')   || '[]'); }
...
// buscarDiscos ahora filtra en localStorage en vez de llamar a la API
async function buscarDiscos(q, pagina = 1) {
    ...
    const todos  = getDiscos();
    const filtrados = todos.filter(d =>
        d.titulo.toLowerCase().includes(ql) ||
        (d.artista || '').toLowerCase().includes(ql) ||
        (d.genero  || '').toLowerCase().includes(ql)
    );

    const porPagina = ITEMS_POR_PAGINA;
    const inicio    = (pagina - 1) * porPagina;
    const resultados = filtrados.slice(inicio, inicio + porPagina);
    const paginas    = Math.ceil(filtrados.length / porPagina);

    if (countEl) countEl.textContent = `${filtrados.length} resultados`;
    renderizarDiscos(resultados);
    _renderizarPaginacion(paginas, pagina, q);
}
...
// ── MODAL DETALLE ─────────────────────────────────
// Ahora obtiene el disco de localStorage en vez de la API
async function abrirModalDetalle(discogsId) {
    ...
    const disco = getDiscos().find(d => d.discogs_id === discogsId);
    if (!disco) {
        document.getElementById('detalle-titulo').textContent = 'Disco no encontrado';
        return;
    }
    discoActivo = disco;
...
async function abrirModalStorytelling(discogsId) {
    ...
    // Mostrar descripción local como "historia"
    ...
    if (storyTexto) {
        storyTexto.textContent = disco.descripcion || `${disco.titulo} es uno de los álbumes más influyentes de ${disco.genero || 'la música'}.`;
    }
}
...
// Recomendaciones: discos del mismo género
async function _cargarRecomendacionesModal(discogsId) {
    ...
    const actual = getDiscos().find(d => d.discogs_id === discogsId);
    if (!actual) return;

    const discos = getDiscos()
        .filter(d => d.discogs_id !== discogsId && d.genero === actual.genero)
        .slice(0, 3);
...
// Video y historia: sin API, simplemente no se muestran
...
// _cargarGenero: filtra en localStorage
async function _cargarGenero(genero, pagina) {
    ...
    // Usamos cache en memoria igual que el original
    ...
    const todos      = getDiscos();
    const filtrados  = todos.filter(d => d.genero === genero);
    const porPagina  = ITEMS_POR_PAGINA;
    const inicio     = (pagina - 1) * porPagina;
    const resultados = filtrados.slice(inicio, inicio + porPagina);
    const paginas    = Math.ceil(filtrados.length / porPagina) || 1;

    const data = { resultados, paginas, pagina };
...
function buscarEnNovedades(q) {
    ...
    _novedadesBusquedaTimeout = setTimeout(() => {
        // Búsqueda local en modo story
...
function cerrarSesion() {
    ...
    localStorage.removeItem('esDemo');
    actualizarInterfazUsuario();
...
// ── PROCESAR PAGO (guarda en localStorage) ────────
async function procesarPago() {
    ...
    if (_discoPagoActivo._esCarrito) {
        ...
        items = [{
            ...
            cantidad:   1,
            subtotal:   Number(_discoPagoActivo.precio),
        }];
    }
    ...
    const nuevaVenta = {
        id_venta:  Date.now(),
        cliente:   { nombre: usuarioLogueado },
        discos:    items,
    ...
    localStorage.setItem('vv_ventas', JSON.stringify(ventas));
...
// Lee el historial desde localStorage filtrando por usuario actual
async function cargarHistorialCompras() {
    ...
    const todasVentas   = getVentas();
    _comprasData   = todasVentas
        .filter(v => v.cliente?.nombre === usuarioActual)
        .reverse();
    _comprasPagina = 1;
    _renderizarCompras();
}
    ...
    contenido.innerHTML = pagina.map(venta => {
        ...
        const discos = (venta.discos || []).map(d => `
            <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--border-dim);font-size:0.8rem;">
                ...
                <span style="color:var(--amber);font-weight:600;white-space:nowrap;">$${Number(d.subtotal).toFixed(2)}</span>
            </div>`).join('');
            ...
            <div style="padding:16px 20px;border-bottom:1px solid var(--border-subtle);">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                    ...
                    <div style="text-align:right;">
                        <div style="font-weight:700;color:var(--amber);">$${Number(venta.total).toFixed(2)}</div>
